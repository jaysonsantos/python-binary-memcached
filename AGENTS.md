...
- `python -m pip install -e . && python -m pip install --group test --group lint`: install the
  package in editable mode with the `test` and `lint` dependency groups from `pyproject.toml`.
