...
class TestBadResponseHeader(unittest.TestCase):
    ...
    def testMalformedHeaderRaisesAndDisconnects(self):
        ...
        bad_header = struct.pack(bmemcached.protocol.Protocol.HEADER_STRUCT, 0x80, 0x00, 0, 0, 0, 0, 16, 0, 0)

        with (
            mock.patch.object(
                bmemcached.protocol.Protocol, "_read_socket", side_effect=self._read_socket_returning(bad_header)
            ),
            mock.patch.object(bmemcached.protocol.Protocol, "_open_connection"),
        ):
    ...
    def testShortHeaderRaisesSocketError(self):
        ...
        """
        with (
            mock.patch.object(bmemcached.protocol.Protocol, "_read_socket", side_effect=OSError("short read")),
            mock.patch.object(bmemcached.protocol.Protocol, "_open_connection"),
        ):
            self.server.connection = mock.Mock()
